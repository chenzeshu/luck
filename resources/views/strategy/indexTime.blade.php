    ...
    @if(count($data)>0)
        @foreach($data as $stock)
            <tr>
                ...
                <td>{{$stock['msg']}}</td>
                <td>{{$stock['refertime']}}</td>
                <td>
                    @include('strategy._updateTime')
                    <button class="uk-button uk-button-danger" onclick="delVT({{$stock['id']}})">删除策略</button>
                    @if($stock['known']===0)
...
<ul class="uk-pagination">
    @if($curPage > 1)
        <li><a href="{{url('v1/s/t')."/".($curPage - 1)."/10"}}"><i class="uk-icon-angle-double-left"></i></a></li>
    @else
    ...
    @foreach(range(1, $pageCount) as $num)
        ...
        @elseif(  abs($num - $curPage) < 3 || $num < 3 || $pageCount - $num < 2 )
            <li class="uk-disabled"><a href="{{url('v1/s/t/')."/".($num)."/10"}}"><span>{{$num}}</span></a></li>
    ...
    @else
        <li><a href="{{url('v1/s/t')."/".($curPage + 1)."/10"}}"><i class="uk-icon-angle-double-right"></i></a></li>
    @endif
</ul>
@endsection
...
@section('customerJS')
    <script>
        function editValue(id) {
            let msg = $("#editVTmsg-"+id).val()
            let val = $("#editVTvalue-"+id).val()
            ...
            if(val != null && val != ""){
                $.post("{{url('v1/s/t/update')}}", {id, msg, val, _token:"{{csrf_token()}}"}, function (res) {
                    if(res.errno == 0){
                        UIkit.notify({
                            message : res.msg,
                            status  : 'info',
        ...
        function delVT(id) {
            ...
            $.get("{{url('v1/s/t/delete')}}/"+id, function (res) {
                if(res.errno == 0){
                    UIkit.notify({
                        message : res.msg,
                        status  : 'info',
        ...
        function known(id) {
            ...
            $.get("{{url('v1/s/t/known')}}/"+id, function (res) {
                if(res.errno == 0){
                    UIkit.notify({
                        message : res.msg,
                        status  : 'info',

...
@section('content')
    ...
    <h2>月金叉推荐： 默认显示 <b style="color:red">3个月</b>内最后一个月线叉为金叉的股票</h2>
    ...
    <ul class="uk-pagination">
        @if($curPage > 1)
            <li><a href="{{url('v1/rec/getmonth')."/".($curPage - 1)."/15/"}}"><i class="uk-icon-angle-double-left"></i></a></li>
        @else
        ...
        @foreach(range(1, $pageCount) as $num)
            ...
            @elseif(  abs($num - $curPage) < 3 || $num < 3 || $pageCount - $num < 2 )
                <li class="uk-disabled"><a href="{{url('v1/rec/getmonth/')."/".($num)."/15/"}}"><span>{{$num}}</span></a></li>
        ...
        @else
            <li><a href="{{url('v1/rec/getmonth')."/".($curPage + 1)."/15/"}}"><i class="uk-icon-angle-double-right"></i></a></li>
        @endif
    </ul>
@endsection
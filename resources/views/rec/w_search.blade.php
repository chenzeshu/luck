...
@section('content')
    <h2>周金叉推荐： 默认显示距今 <b style="color:red">{{getC('diff_w')}} - {{getC('diff_w2')}}个月</b>内最后一个周线叉为金叉的股票</h2>
    ...
    <ul class="uk-pagination">
        @if($curPage > 1)
            <li><a href="{{url('v1/rec/wsearch')."/".($curPage - 1)."/15"}}"><i class="uk-icon-angle-double-left"></i></a></li>
        @else
        ...
        @else
            <li><a href="{{url('v1/rec/wsearch')."/".($curPage + 1)."/15"}}"><i class="uk-icon-angle-double-right"></i></a></li>
        @endif
    </ul>
@endsection
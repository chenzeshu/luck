...
@section('content')
    <h2>周金叉推荐： 默认显示 <b style="color:red">3个月</b>内最后一个周线叉为金叉的股票</h2>
    <div class="stock-search" onkeydown="mytest()">
        <form action="{{url('v1/rec/wsearch/1/15')}}" method="post" class="uk-form">
            {{csrf_field()}}
    ...
    <ul class="uk-pagination">
        @if($curPage > 1)
            <li><a href="{{url('v1/rec/getweek')."/".($curPage - 1)."/15"}}"><i class="uk-icon-angle-double-left"></i></a></li>
        @else
        ...
        @foreach(range(1, $pageCount) as $num)
            ...
            @elseif(  abs($num - $curPage) < 3 || $num < 3 || $pageCount - $num < 2 )
                <li class="uk-disabled"><a href="{{url('v1/rec/wsearch/')."/".($num)."/15"}}"><span>{{$num}}</span></a></li>
            @elseif( abs($num - $curPage) > 3 )
                ...
            @elseif(abs($num - $curPage) == 3)
                <li><a href="#"><span>...</span></a></li>
            @else
                <li class="uk-disabled"><a href="{{url('v1/rec/getweek')."/{$num}/15"}}"><span>{{$num}}</span></a></li>
            @endif
        @endforeach
        ...
        @else
            <li><a href="{{url('v1/rec/getweek')."/".($curPage + 1)."/15"}}"><i class="uk-icon-angle-double-right"></i></a></li>
        @endif
    </ul>
@endsection